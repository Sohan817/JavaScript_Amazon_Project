...
export class Product {
  ...
  constructor(prodctDetails) {
    this.id = prodctDetails.id;
    this.image = prodctDetails.image;
    this.name = prodctDetails.name;
    this.rating = prodctDetails.rating;
    this.priceCents = prodctDetails.priceCents;
    this.keywords = prodctDetails.keywords;
  }
  ...
  getPrice() {
    return `$${foratCurrency(this.priceCents)}`;
  }
...
export class Clothing extends Product {
  ...
  constructor(prodctDetails) {
    super(prodctDetails);
    this.sizeChartLink = prodctDetails.sizeChartLink;
  }
...
export class Appliance extends Product {
  ...
  constructor(prodctDetails) {
    super(prodctDetails);
    this.instructionsLink = prodctDetails.instructionsLink;
    this.warrantyLink = prodctDetails.warrantyLink;
  }
  extraInfoHTML() {
...
//Make http request by fetch()
export function loadProducsFetch() {
  const prmise = fetch("https://supersimplebackend.dev/products")
    .then((response) => {
      return response.json();
    })
    .then((productsData) => {
      products = productsData.map((prodctDetails) => {
        if (prodctDetails.type === "clothing") {
          return new Clothing(prodctDetails);
        }
        if (prodctDetails.type === "appliances") {
          return new Appliance(prodctDetails);
        }
...
//Make http request by XMLHttpRequest
export function loadProducs(fun) {
  const xhr = new XMLHttpRequest();
  xhr.addEventListener("load", () => {
    products = JSON.parse(xhr.response).map((prodctDetails) => {
      if (prodctDetails.type === "clothing") {
        return new Clothing(prodctDetails);
      }
      if (prodctDetails.type === "appliances") {
